...
class MeMoBuilder():
    ...
    def from_cli_args(arguments):
        """
        Creates a MeMoBuilder from the given argument strings. The caller must provide
        an array of string arguments like this:

        usage: memo_builder.py [-h] [--yaml [YAML_FILE]] [--sampler | --no-sampler]
                       [--trainer | --no-trainer]
                       H5File
    ...
    def parse_args(arguments):
        """
        This function uses the argparse module to convert the given argument string to objects and assigns them as
        attributes to a namespace.
        ...
        parser.add_argument(
            'H5File',
            type=str,
            help='HDF5 file that will be used as storage for configuration, sampled data and trained models. '
        ...
        parser.add_argument(
            ...
            nargs='?',
            type=str,
            help='YAML configuration file. If present a new HDF5 file will be created and the configuration '
                 'will be imported. Overwrites existing HDF5 datastores.'
        )

        sampling_group = parser.add_mutually_exclusive_group(required=False)
        sampling_group.add_argument('--sampler', dest='sampler', action='store_true')
        sampling_group.add_argument('--no-sampler', dest='sampler', action='store_false')
        parser.set_defaults(sampler=True)

        trainer_group = parser.add_mutually_exclusive_group(required=False)
        trainer_group.add_argument('--trainer', dest='trainer', action='store_true')
        trainer_group.add_argument('--no-trainer', dest='trainer', action='store_false')
    ...
    def run(self, custom_adapters={}):
        ...
        If no --yaml parameter is given, the program assumes that the configuration data was already imported,
        and loads if from the hdf datastore.

        Running the sampler involves invoking a mosaik simulator, with all necessary parameters. Mosaik allows it's
        users freely to specify how to pass parameter values to their models. Thus, it may happen, that the
        parameters must be wrapped in dictionaries or in specialized objects in order to initialize a simulator or
