"""This module contains score functions of the memo trainer."""
import numpy as np
import math
...
def r2_score(approximation_model, test_data):
    """
    Computes the r² score of the *approximation_model* applied to the
    previously configured test data in *self.y_pred*.
    ...
    if test_data.responses.shape[1] > 1:
        ...
        scorer = metrics.make_scorer(metrics.r2_score)
    score_value = scorer(approximation_model, test_data.inputs,
                         test_data.responses)
    return score_value
...
def mae_score(approximation_model, test_data):
    """
    Computes the mean average error of the *approximation_model*
...
def harmonic_averages_error(y_true, y_pred, multioutput='uniform_average'):
    """Computes the harmonic average as an optimistic error function"""
