...
from memobuilder.mdb.h5db.core import H5AccessMode
...
def export_model_to_hdf(target_file, model_structure, metamodel):
    ...
    if model_type in linear_models:
        raise Exception(
            'umgebaut, damit alle mit dem genersischen export behandelt werden')
        ...
        pipeline = metamodel.regression_pipeline
        estimator = pipeline
        export_kernel_ridge_regression_model_to_hdf(
            target_file, model_structure, estimator)
    else:
        pipeline = metamodel.regression_pipeline
        estimator = pipeline
        export_generic_sklearn_model_to_hdf(
            target_file, model_structure, estimator)
        print('WARNING: using generic export morde for models of type "%s" ' %
              (str(type(metamodel))))
