...
class TerminalController():
    ...
    def open_db(self):
        ...
        if yaml_file:
            ...
            self.model.open_db(self.h5_file, access_mode=h5db.H5AccessMode.WRITE_TRUNCATE_ON_EXIST)
        else:
            # open, but keep existing content
            self.view.announce_database_truncated(False)
            self.model.open_db(self.h5_file, access_mode=h5db.H5AccessMode.READ_WRITE_EXISTING_FILE)
    ...
    def run_sampler(self, custom_adapters={}):
        ...
        for sampler_conf in sampler_configurations:
            ...
            if sid in self.model.sampling_results:
                ...
                self.view.announce_sampler_status(sid, 'skipped (data is already present)')
                continue
    ...
    def load_configuration(self):
        # always load configuration from HDF5, such that each object has an ID
        self.view.announce_configuration_source('HDF5')
        self.model.load_configuration_from_db()

    def update_sampling_results(self):
        # update the model's state, such that it is aware of all stored sampling result objects
        self.model.update_sampling_results()

    def update_training_results(self):
        # update the model's state, such that it is aware of all stored training result objects
        self.model.update_training_results()

    def run_trainer(self):
        trainer_configurations = self.model.get_surrogate_model_configurations()
        if len(trainer_configurations) == 0:
            # announce that no trainer configuration was found
            self.view.announce_training_error('no surrogate model training configuration found')
        for trainer_configuration in trainer_configurations:
            ...
            if self.trainer_result_exists(name):
                self.view.announce_trainer_status(tid, 'skipped (data is already present)')
                continue
