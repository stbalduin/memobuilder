"""
This module facilitates the sampling process by encapsulating the setup
of a mosaik sampling scenario. The class :class:`SamplingScenario` is
responsible for the creation of a mosaik world and populating it with
simulators. The class :class:`SourceSimulator` represents simulators
which provide input data at simulation time and the
:class:`SinkSimulator` represents a storage for output data that is
also generated at simulation time.
...
import mosaik
from memobuilder.msampler.simulators import Collector, InputProvider
...
class SamplingScenario():
    """Sampling scenario class """
    # Name for the simulator under investigation in the mosaik scenario
    EXPERIMENT_SIM_NAME = 'SIM'

    # Name for the simulator that provides inputs for the simulator
    # under investigation in the mosaik scenario.
    IN_SIM = 'INPUT_SIM'

    # Name for the simulator that collects outputs of the simulator
    # under investigation in the mosaik scenario.
    OUT_SIM = 'COLLECTOR_SIM'

    # Simulator configuration.py for the execution of an experiment
    # in the mosaik environment. A copy of this sim_config will be
    # updated when the configuration.py of the simulator under
    # investigation is provided."""
    EXPERIMENT_SIM_CONFIG = {
        # simulator that gathers outputs
        OUT_SIM: Collector.SIM_CONFIG,
        # simulators that provide constant inputs
        IN_SIM: InputProvider.SIM_CONFIG,
    ...
    def __init__(self, sim_config, model_structure):
        ...
        # copy initial sim_config
        self.sim_config = copy.copy(SamplingScenario.EXPERIMENT_SIM_CONFIG)
        # add sim_config
        self.sim_config[SamplingScenario.EXPERIMENT_SIM_NAME] = sim_config
        # name of the simulator
        self.sim_name = SamplingScenario.EXPERIMENT_SIM_NAME
    ...
    def simulate(self, parameter_assignments, duration=STEP_SIZE):
        ...
        # connect outputs
        self.connect_outputs(world, self.collector_adapter,
                             self.model_adapter)
...
class MosaikInputSimulatorAdapter(object):
    ...
    def mosaik_init(self, parameter_assignments):
        self.simulator = self.world.start(SamplingScenario.IN_SIM,
                                          step_size=STEP_SIZE)

    def mosaik_create(self, parameter_assignments):
        inputs = {attr_name: parameter_assignments[attr_name]
                  for attr_name in self.model_structure.model_inputs}
        input_values = self.preprocess_input_values(
            inputs, parameter_assignments, self.model_structure
        )
    ...
    def preprocess_input_values(self, input_values, parameters,
                                model_structure):
        return input_values
...
class MosaikOutputCollectorAdapter(object):
    ...
    def mosaik_init(self, parameter_assignments):
        self.simulator = self.world.start(SamplingScenario.OUT_SIM,
                                          step_size=STEP_SIZE)
    ...
    def postprocess_output_values(self, output_values, parameters,
                                  model_structure):
...
class MosaikSimulatorAdapter(object):
    ...
    def mosaik_init(self, parameter_assignments):
        sim_params = {param: parameter_assignments[param]
                      for param in self.model_structure.simulator_parameters}
        self.__fix_step_size_datatype(sim_params)
        sim_param_values = self.preprocess_simulator_parameters(
            sim_params, parameter_assignments, self.model_structure
        )
        self.simulator = self.world.start(
            SamplingScenario.EXPERIMENT_SIM_NAME, **sim_param_values
        )

    def mosaik_create(self, parameter_assignments):
        model_params = {param: parameter_assignments[param]
                        for param in self.model_structure.model_parameters}
        model_param_values = self.preprocess_model_parameters(
            model_params, parameter_assignments, self.model_structure
        )

        # find the 'ModelMock' object, which allows access to the sim
        mocks = [m for a, m in self.simulator.__dict__.items()
                 if type(m) == ModelMock]
        if len(mocks) > 1:
    ...
    def preprocess_simulator_parameters(self, sim_params, parameters,
                                        model_structure):
        return sim_params

    def preprocess_model_parameters(self, model_params, parameters,
                                    model_structure):
        return model_params

    def __fix_step_size_datatype(self, sim_param_assignments):
        """Mosaik does not accept 'numpy.int64' as type of the
        next_step value, which is returned by simulators after each
        step. Solution here is to just convert type of step_size
        parameters, if present.
        """
        if 'step_size' in sim_param_assignments:
            sim_param_assignments['step_size'] = int(
                sim_param_assignments['step_size']
            )
