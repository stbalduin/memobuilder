"""This module contains factory functions for the memosampler."""
import collections
...
def create_sampler(sampler_configuration):
    """
    Factory method that create a new sampler object based on the
    information from a memomodel.SamplerConfig object.

    :param sampler_configuration: a fully initialized
    memomodel.SamplerConfig object with simConfig, modelStructure,
    ...
    param_variation_model = create_parameter_variation_model(
        sampler_configuration.parameter_variations
    )

    # create sampling_strategy
    strategy = create_sampling_strategy(param_variation_model,
                                        sampler_configuration.strategy)

    # create a sampling scenario
    scenario = create_sampling_scenario(sampler_configuration.sim_config,
                                        sampler_configuration.model_structure)
...
def create_parameter_variation_model(parameter_variations):
    """Factory method that create a
    memosampler.sampler.ParameterVariationModel from a list of
    memomodel.ParameterVariation objects.

    Interprets ParameterVariation objects from the configuration and
    maps them to corresponding Variabilty objects, that the sampler
    understands. Constants and design variables will be provided in
    two separated dictionaries, which map the parameter or attribute
    name to the variability object.

    :param parameter_variations: List of memomodel.ParameterVariation
    objects
...
class RecursiveSubClassFinder(object):
    ...
    def find_subclasses(superclazz):
        subclass_map = {clazz.__name__: clazz
                        for clazz in superclazz.__subclasses__()}
        subclasses = list(subclass_map.values())
        for subclass in subclasses:
            subclass_map.update(
                RecursiveSubClassFinder().find_subclasses(subclass)
            )
        return subclass_map
...
def create_sampling_scenario(sim_config, model_structure):
    """A factory method that creates a
    memosampler.scenario.SamplingScenario from a memomodel.SimConfig
    and a memomodel.ModelStructure configuration object.

    :param sim_config: A memomodel.SimConfig configuration object

    :param model_structure: A memomodel.ModelStructure configuration
    object

    :return: a memosampler.scenario.SamplingScenario object, ready for
    experimentation
    """
