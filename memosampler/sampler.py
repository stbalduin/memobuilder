...
class Sampler(object):
    ...
    def run(self):
        # this is the main loop of the sampling process, where the sampling scenario is invoked repeatedly with
        # varying inputs. It is up to the sampling strategy in use to decide when the sampling process is finished
        # and how to draw input samples.

        # prepare a dictionary with constant parameters for later reuse
        constants = {}
        for attr_name, variability in self.parameter_variation_model.design_constants.items():
            constants[attr_name] = variability.value

        # initialize an empty sampling result object (column names needed here).
        input_names = list(self.parameter_variation_model.design_variables.keys())
        response_names = list(self.model_structure.model_outputs)
        sampling_result = memomodel.InputResponseDataset(input_cols=input_names, response_cols=response_names)
        ...
        while not self.strategy.is_done(sampling_result):
            ...
            # populate the current parameter assignments with the constant parameters and the current sample
