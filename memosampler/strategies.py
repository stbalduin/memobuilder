...
class SamplingStrategy(object):
    ...
    def is_done(self, sampling_result):
        raise Exception('This method is supposed to be implemented by subclasses')
...
class FixedDesignBasedSamplingStrategy(SamplingStrategy):
    ...
    def get_next_sample(self):
        ...
        for (param_name, variability), value in zip(self.param_variation_model.design_variables.items(), experiment):
...
class FullFactorial(FixedDesignBasedSamplingStrategy):
    ...
    def init(self):
        num_levels = []
        print(self.param_variation_model.design_variables)
        for attr_name, variability in self.param_variation_model.design_variables.items():
            if not isinstance(variability, (memosampler.NumericalLevels, memosampler.NonNumericalLevels)):
                raise ValueError('FullFactorial can only handle "NumericalLevels"  or "NonNumericalLevels" types of '
                                 'variability')
...
class LHS(FixedDesignBasedSamplingStrategy):
    ...
    def __init__(self, param_variation_model, num_samples, **kwargs):
        ...
        FixedDesignBasedSamplingStrategy.__init__(self, param_variation_model, **kwargs)
