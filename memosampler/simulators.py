"""
This module contains some simulators that are used by other components of MeMoSampler.
...
* InputProvider is a simulator which provides inputs to the simulator under investigation at simulation time.
  The input value may be of an arbitrary complex data type that the simulator under investigation is able to
  process. If simulated over several steps of time, the provided value does not change - InputProvider was designed
  for single step experiments.

* Collector simply collects the values of all incoming connections. The simulator has an extra function
  :meth:`get_monitored_data` which returns a list of DataFrames at runtime. One for each connected entity.
  If simulated over several steps of time the collector always collects all connected values. Thus the collector
  simulator has been used in single step experiments and in scenarios built for the comparison of simulator and
  surrogate simulator.

* TestSimulator is a simulator used in tests and during development of InputProvider and Collector simulators.
...
class InputProvider(mosaik_api.Simulator):
    ...
    def __init__(self):
        meta = copy.copy(InputProvider.META)  # create a copy of global meta_data data
        super().__init__(meta)  # meta_data data
        self.eid_prefix = InputProvider.EID_PREFIX  # prefix for eids
        self.step_size = None  # step size of the simulation
        self.data = dict()  # maps eid to value
    ...
    def create(self, num, model, value):
        ...
        :param value: object
            an object which will be returned after each call to the method :meth:`get_data` of the new model instance.
        ...
        for i in range(next_eid, next_eid + num):
            ...
            # no model needs to be created. just store the value in a dict which maps eids to the given values
            self.data[eid] = value
...
class SeriesInputProvider(mosaik_api.Simulator):
    ...
    def __init__(self):
        meta = copy.copy(SeriesInputProvider.META)  # create a copy of global meta_data data
        super().__init__(meta)  # meta_data data
...
class Collector(mosaik_api.Simulator):
    """
    This simulator provides a data collector which accepts any given inputs from other models and collects their values.
    The *extra_method* :meth:`get_monitored_data` provides access to the monitored data.
    """
    ...
    def __init__(self):
        meta = copy.copy(Collector.META)  # create a copy of global meta_data data
        super().__init__(meta)  # meta_data data
    ...
    def create(self, num, model, container=dict()):
        ...
        :param container: dict
            an empty dictionary which will be filled with data during simulation.
    ...
    def get_data(self, outputs):
        ...
        :raises: Exception because this simulator is not supposed to be connected as input for other simulators.
        """
    ...
    def get_monitored_data(self):
        ...
        :return: a dictionary which maps the full entity ids of source simulators to DataFrames containing the monitored
