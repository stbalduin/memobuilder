...
{{ only_html }}
   ...
   {% if source_link and (html_show_formats and not multi_image) %}
   (
   {%- if source_link -%}
...
def run_code(code, code_path, ns=None, function_name=None):
    ...
    # FIXME This doesn't seem to work if we import modules in the example
    # relative to the current (old) pwd. The specific case where this failed is
    # as follows:
            ...
            for format, dpi in formats:
                try:
                    if 'figsize' in options:
                        size = [float(s)
                            for s in options['figsize'].split(' ')]

                        figman.canvas.figure.set_size_inches(size)
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    # see note in Python docs for warning about.rst symbolic links on Windows.
    ...
    try:
        results = render_figures(code, source_file_name, build_dir, output_base,
                                 context, function_name, config, options)
        errors = []
